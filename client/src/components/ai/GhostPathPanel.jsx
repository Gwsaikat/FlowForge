...
export default function GhostPathPanel({ projectId, onClose, onHighlight }) {
  ...
  return (
    <motion.div className="ai-panel ghost-panel" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <div className="ai-panel-header">
        <div className="ai-panel-title">
          <Ghost size={20} className="ghost-icon" />
          <div>
            <h3>Ghost Critical Path</h3>
            <span className="ai-badge unique">World First</span>
          </div>
        </div>
        ...
        <div className="ai-panel-body">
          <div className="ghost-risk-meter">
            ...
            <div className="ghost-meter-track">
              <motion.div
                className="ghost-meter-fill"
                initial={{ width: 0 }}
                animate={{ width: `${data.shiftRisk}%` }}
          ...
          {data.ghostTasks?.length > 0 && (
            <button className="btn btn-primary" onClick={() => onHighlight?.(data.ghostTasks.map((g) => g.taskId))}>
              Highlight Ghost Tasks
            </button>
          )}
          {!data.ghostTasks?.length && (
            <div className="ghost-safe">✅ No ghost path shifts predicted</div>
          )}
        </div>
      ) : null}
    </motion.div>
